...
public class OrderFulfillmentActivityImpl implements OrderFulfillmentActivity {
    ...
    public void restockInventory(Order order) {
        ...
        inventoryServiceApi.releaseInventory(order.getOrderId().toString());
    }

    @Override
    public void sendShippingMessage(final Order order) {
        log.info("➡️ SendShippingMessage invoked");
        CompletableFuture<SendResult<UUID, ShippingEvent>> messagePostFuture = shippingRequestKafkaTemplate.send(Topics.TOPIC_ORDER_SHIPPING_REQUESTED, order.getOrderId(), ORDER_TO_SHIPPING_EVENT.apply(order));
        messagePostFuture.whenComplete((result, ex) -> {
            if (ex == null) {
                RecordMetadata recordMetadata = result.getRecordMetadata();
                log.info("Order id {} posted to {}:{}:{}", order.getOrderId(), recordMetadata.topic(), recordMetadata.partition(), recordMetadata.offset());
            }
        });
    }
