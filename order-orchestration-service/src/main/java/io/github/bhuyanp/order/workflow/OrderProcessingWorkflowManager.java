...
public class OrderProcessingWorkflowManager {
    ...
    public Order start(final CreateOrderRequest orderRequest) {
        ...
        return workflow.createOrder(orderRequest);
    }
    ...
    public void updateShippingDetails(ShippingCompletionEvent shippingCompletionEvent) {
        try {
            String workflowId = getOrderFulfillmentWorkflowId(shippingCompletionEvent.orderId());
    ...
    public List<String> getWorkflowStatus(UUID orderId) {
        return client.streamHistory(getOrderFulfillmentWorkflowId(orderId)).map(historyEvent -> historyEvent.getActivityTaskScheduledEventAttributes().getActivityType().getName()).filter(StringUtils::hasText).toList();
    }

    private String getOrderFulfillmentWorkflowId(UUID orderId) {
